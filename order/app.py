...
def lambda_handler(event, context):
    ...
    if eventType is not None:
        ...
        if eventType == 'CreateOrder':
            ...
            process_result(result, 'OrderDelivered', 'ErrorOrderDelivered', order, None, EVENT_BUS, EVENT_SOURCE)
        elif eventType == 'DeliveryWasCanceled':
            result = update_order('DELIVERY_CANCELED', order)
            process_result(result, 'OrderCanceled', 'ErrorOrderCanceled', order, 'order', EVENT_BUS, EVENT_SOURCE)
...
def store_order(orderStatus, order):
    ...
    params = {
        "Statement": f"INSERT INTO \"{ORDER_TABLE}\" VALUE {json.dumps(db_order)}"
    }
